...
module.exports = class Good extends Component {
  ...
  load (element) {
    ...
    var onscroll = nanoraf(function () {
      ...
      else if (tail) fraction = 1 - (top + size - scrollY) / ((innerHeight / 2) - (width / 2) + border)
      ...
      if (tail) {
        if (!ontail) sticky.classList.add('is-reversed')
        ontail = true
      } else {
        if (ontail) sticky.classList.remove('is-reversed')
        ontail = false
      }
  ...
  createElement (props) {
    return html`
      <div class="Good">
        ...
        <div class="Good-row js-row">
          <svg class="Good-heading js-" width="321" height="655" viewBox="0 0 321 655">
