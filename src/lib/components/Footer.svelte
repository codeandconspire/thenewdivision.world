<script>
  ...
  // Ported from components/footer/index.js
  let { data, languages = [], themed = false, language = 'en', text } = $props()

  function top () {
    requestAnimationFrame(function () { window.scrollTo(0, 0) })
  }

  const columns = $derived([data.col2, data.col3, data.col4])
</script>
        ...
        <ul class="Footer-col">
          {#each col || [] as item}
            <li class="Footer-item">
              <A link={item.link} class="Footer-link" onclick={top}>
                {#if item.icon}<Icon name={item.icon} class="Footer-icon" mono={themed} /> {/if}{item.text}
              </A>
            </li>
          {/each}
        </ul>
      {/each}
      <address class="Footer-col"><RichText field={data.col5} /></address>
      <div class="Footer-col"><RichText field={data.col6} /></div>
    </div>
    {#if languages.length > 1}
      <ol class="Footer-languages">
        {#each languages as lang}
          <li class="Footer-language">
            <a href={lang === 'en' ? '/' : `/${lang}`} class="Footer-link {language === lang ? 'is-active' : ''}">{lang.toUpperCase()}</a>
          </li>
        {/each}
      </ol>
    {/if}
