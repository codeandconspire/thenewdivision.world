...
module.exports = get('/media/:type/:transform/:uri(.+)', async function (ctx, type, transform, uri, next) {
  // Handle prismic embedding an encoded query string
  let [url, encoded, query] = decodeURIComponent(uri).split('?')
  if (encoded) {
    if (query) url += encodeURIComponent(`?${encoded}`)
    else query = encoded
  }
  if (query) url += `?${query}`

  if (transform === '_') transform = null
  const match = url.match(TYPE_REG)
  if (match) {
    type = match[1]
    url = match[2]
  }
